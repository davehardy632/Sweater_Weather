...
describe Coordinate do
  it "is instantiated with google geocode service" do
  coordinate_data = {:results=>
  [{:address_components=>
 ...
 :status=>"OK"}
    ...
    expect(coordinate.id).to eq("ChIJzxcfI6qAa4cR1jaKJ_j0jhE")
    expect(coordinate.location).to eq("Denver Colorado")
    expect(coordinate.latitude).to eq(39.7392358)
    expect(coordinate.longitude).to eq(-104.990251)
  end
end
